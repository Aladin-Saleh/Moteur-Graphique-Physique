...
#error Invalid VMA_VULKAN_VERSION.
	...
	bool BufferManager::createImageBuffer(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VmaBufferImage& image, VulkanDeviceMisc * vM)
	{
		...
		if (vkCreateImage(vM->str_device, &imageInfo, nullptr, &image.image) != VK_SUCCESS)
		{
		...
		VkMemoryRequirements memRequirements;
		vkGetImageMemoryRequirements(vM->str_device, image.image, &memRequirements);
