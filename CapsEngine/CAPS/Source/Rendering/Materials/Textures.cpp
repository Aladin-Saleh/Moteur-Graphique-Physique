...
namespace Ge
{
	Textures::Textures(stbi_uc* pc, int Width, int Height,int ind, VulkanMisc * vM)
	{
		...
		BufferManager::createImageBuffer(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, vM->str_VulkanDeviceMisc);
		...
		textureImageView = ImageViews::createImageView(textureImage.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels, vM->str_VulkanDeviceMisc);
	...
	Textures::~Textures()
	{
		vkDestroySampler(vMD->str_device, textureSampler, nullptr);
		vkDestroyImageView(vMD->str_device, textureImageView, nullptr);
	...
	{
		VkCommandBuffer commandBuffer = CommandBuffers::beginSingleTimeCommands(vM);
		...
		CommandBuffers::endSingleTimeCommands(commandBuffer, vM);
	}
