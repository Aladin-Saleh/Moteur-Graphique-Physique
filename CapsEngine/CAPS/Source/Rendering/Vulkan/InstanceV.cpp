...
namespace Ge
{
	std::vector<const char*> InstanceV::instanceExtensions =
	{
		VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME
	};
	bool InstanceV::InitInstance(const char * name, version v, VulkanDeviceMisc * vDM)
	{
		...
		appInfo.pEngineName = "Caps Engine";
		...
		{
			Debug::Error("echec de la cr�ation de l'instance vulkan");
			return false;
		}
		vDM->str_instance = instance;
		Debug::Info("Initialisation de l'instance vulkan");
		return true;
	}

	void InstanceV::Release()
	{
		vkDestroyInstance(InstanceV::instance, nullptr);
		Debug::Info("Liberation de l'instance vulkan");
	}
}