...
namespace Ge
{
	...
	bool Caps_HUD::Init_HUD(VulkanMisc* vM, Model_Manager * mm, LightManager * lm, PhysicsEngine * pe, SoundEngine * se, CameraManager * cm)
	{
		IMGUI_CHECKVERSION();
		isHUDActive = true;
		...
		m_SoundEngine = se;
		m_CameraManager = cm;
		m_hiearchyGUI = new HiearchyGUI(mm,lm,pe,se,cm);
		m_projectGUI = new ProjectGUI();
		return true;
	}

	void Caps_HUD::check_vk_result(VkResult err)
	{
		if (err != VK_SUCCESS)
		{
	...
	bool Caps_HUD::createCommandPool(VkCommandPool* commandPool, VkCommandPoolCreateFlags flags, VulkanDeviceMisc* vM)
	{
	...
	bool Caps_HUD::createCommandBuffers(VkCommandBuffer* commandBuffer, uint32_t commandBufferCount, VkCommandPool& commandPool, VulkanDeviceMisc* vM)
	{
	...
	void Caps_HUD::ReleaseHUD(VkDevice device)
	{
		delete(m_projectGUI);
	...
	void Caps_HUD::RecreateSwapChain(VulkanMisc* vM)
	{
	...
	void Caps_HUD::Render(VulkanMisc* vM, uint32_t currentframe)
	{
	...
	void Caps_HUD::ImGuiRender(VulkanMisc* vM)
	{
		bool openInfo = true;
	...
	void Caps_HUD::Console_Debug(VulkanMisc* vM)
	{
		bool* openConsole = nullptr;
		m_console_HUD.Draw("Console", vM, openConsole);
	}

	/*
	* Cette méthode permet d'indiquer à ImGui d'ajouter une barre de menu
	* On ajoute ensuite les MenuItem qui sont des item avec chacun une action
	*/
	void Caps_HUD::MenuBar(bool* p_open,bool* is_UI_is_active)
	{
		bool test = true;
		...
		if (ImGui::BeginMainMenuBar())
		{
			...
			{
				if (ImGui::MenuItem("Change scene(en developpement)")) {}
				if (ImGui::MenuItem("Ajouter un script(en developpement)")) {
					//system("copy  C:\\Users\\Aladin Saleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Script\\Source\\ScriptSchema.cpp  C:\\Users\\Aladin Saleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Script\\Source\\New_Script.cpp");
					//system("explorer.exe ");
				//	system("cp \"C:\\Users\\AladinSaleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Script\\Source\\TestPlugin.cpp"" \"C:\\Users\\AladinSaleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Script\\Source\\New_Script.cpp");

					//system("explorer.exe \"C:\\Users\\Aladin Saleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Script\\Source"); //SW_SHOW);
					Debug::Warn("En developpement");
				}

				if (ImGui::MenuItem("Ajouter une scene(en developpement)")) {
					
					//system("explorer.exe \"C:\\Users\\Aladin Saleh\\Repertoire_de_travaille\\C++\\PT2020-CASP\\CapsEngine\\CAPSDLL\\Scene\\Source"); //SW_SHOW);
					Debug::Warn("En developpement");
			...
			if (ImGui::BeginMenu("Options"))
			{
				...
				if (ImGui::BeginMenu("Afficher interface scene")) {
					if (ImGui::MenuItem("Activer/Désactiver", "", is_UI_is_active, is_UI_is_active)) {
						
					}
					ImGui::EndMenu();
				}

				if (ImGui::BeginMenu("Style interface")) {
					if (ImGui::MenuItem("Dark")) {
						ImGui::StyleColorsDark();
					}

					if (ImGui::MenuItem("Classqiue")) {
						ImGui::StyleColorsClassic();
					}
					ImGui::EndMenu();
				}
	...
	void Caps_HUD::InfoSystem(VulkanMisc* vM)
	{
		/*
		* Méthode à completer
		*/
	...
	void Caps_HUD::OverlayInfo(VulkanMisc* vM, bool* p_open)
	{
		ImGuiIO& io = ImGui::GetIO();
		...
		ImGui::SetNextWindowPos(ImVec2(vM->str_VulkanSwapChainMisc->str_swapChainExtent.width - 330.0f, 20), ImGuiCond_Always);
		...
		if (ImGui::Begin("Example: Simple overlay", p_open, window_flags))
		{
			...
			Ips();
			ImGui::Text("Temps depuis démarage %.1f", ImGui::GetTime());
			ImGui::Text("Nombre de modèles : %d", vM->str_VulkanDescriptor->modelCount);
	...
	void Caps_HUD::HierachyAffichage() {

		/*if (test) {
			Debug::Warn("Modele avec un nom : ");
			for (auto& gobj : m_gObjects) {
				if (gobj->hasAName()) {
					Debug::Warn(gobj->GetName().c_str());
				}
			}
			test = false;
		}*/
		...
		/*for (auto& gobj : m_gObjects) {
			if (gobj->hasAName()) {
				if (filter.PassFilter(gobj->getName()) ) {
					ImGui::BulletText("%s", gobj->getName() );
				}
	...
	void Caps_HUD::Ips()
	{
		...
		if (io.Framerate < 27.0f)
		{
			color = ImVec4(1.0f, 0.0f, 0.0f, 1.0f);
		}
		ImGui::TextColored(color, "Image par seconde : %.1f (IPS)", io.Framerate);
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), "Temps entre chaque image  : %.3f ms/frame", 1000.0f / io.Framerate);
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), "Nombre d'images depuis le debut  : %d images", ImGui::GetFrameCount());

		
	}

	void Caps_HUD::UpdateHierachy() {
		m_gObjects.clear();
		m_model = m_modelLoader->getModels();
		for (MShape* ms : m_model) {
			m_gObjects.push_back(ms);
		}
	}

	void Caps_HUD::SetModelManager(Model_Manager* mM)
	{
		m_modelLoader = mM;
	}

	void Caps_HUD::setActiveUI(bool state)
	{
		m_GlobalGUIActive = state;
	}

	ConsoleDebug * Caps_HUD::getConsoleDebug()
	{
		return &m_console_HUD;
	}

	/*void set_b_HUD_active()
	{
		is_UI_is_active = false;
	}*/
